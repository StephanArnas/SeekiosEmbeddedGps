...
void LED_init(void)
{
	...
	gpio_set_pin_pull_mode(LED_2_ye, GPIO_PULL_OFF);
	...
	switch(timer_id)
	{
		case BLE_APP_TIMER_ID1:
		{
			if (!bsp_timer_status[BLE_APP_TIMER_ID1])
			{
				...
				timer_add_task(&TIMER_, &app_timer_1_task);
		...
		{			
			if (!bsp_timer_status[BLE_APP_TIMER_ID2])
			{
				...
				timer_add_task(&TIMER_, &app_timer_2_task);
		...
		{			
			if (!bsp_timer_status[BLE_APP_TIMER_ID3])
			{
				...
				timer_add_task(&TIMER_, &app_timer_3_task);
...
void ble_timer_stop(ble_app_timer_t timer_id)
{
	...
	switch(timer_id)
	{
		case BLE_APP_TIMER_ID1:
		{
			if (bsp_timer_status[BLE_APP_TIMER_ID1])
			{
				timer_remove_task(&TIMER_, &app_timer_1_task);
		...
		case BLE_APP_TIMER_ID2:
		{
			if (bsp_timer_status[BLE_APP_TIMER_ID2])
			{
				timer_remove_task(&TIMER_, &app_timer_2_task);
		...
		case BLE_APP_TIMER_ID3:
		{
			if (bsp_timer_status[BLE_APP_TIMER_ID3])
			{
				timer_remove_task(&TIMER_, &app_timer_3_task);
